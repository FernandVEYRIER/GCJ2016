        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                if (y == 0 && x == 0)
                    ...
                    tmp = (GameObject)Instantiate(left, origin + new Vector3(x * sizeBlockX, y * sizeBlockY, 0), Quaternion.identity);
                else if (x == size.x - 1)
                    tmp = (GameObject)Instantiate(right, origin + new Vector3(x * sizeBlockX, y * sizeBlockY, 0), Quaternion.identity);
                if (tmp != null)
